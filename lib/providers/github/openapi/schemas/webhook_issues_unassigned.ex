defmodule GitHubOpenAPI.WebhookIssuesUnassigned do
  use Ecto.Schema
  import Ecto.Changeset

  @primary_key false
  embedded_schema do
    field :action, Ecto.Enum, values: [:unassigned]
    embeds_one :assignee, GitHubOpenAPI.WebhooksUserMannequin
    embeds_one :enterprise, GitHubOpenAPI.EnterpriseWebhooks
    embeds_one :installation, GitHubOpenAPI.SimpleInstallation
    embeds_one :issue, GitHubOpenAPI.WebhooksIssue
  ...
  def changeset(schema, attrs) do
    ...
    |> cast(attrs, [:action, :__info__, :__joins__])
        |> cast_embed(:assignee, with: &GitHubOpenAPI.WebhooksUserMannequin.changeset/2)
    ...
    |> cast_embed(:issue, with: &GitHubOpenAPI.WebhooksIssue.changeset/2)
